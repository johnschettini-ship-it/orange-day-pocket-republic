...
- [ ] Distinct procedural music states transition safely after browser audio unlock. *(Implemented; needs a human ear — `smoke.js` can only assert the theme id string, not that the audio graph sounds right or crossfades cleanly. Code audit found and fixed two real bugs: `beginChapterIntro()` and the legacy→title transitions called `stopMusic()` right before a state that itself wants a theme, forcing a silent gap instead of `startMusic()`'s built-in crossfade; and the "campaign" theme's sustained bass pad never re-rooted between chapters — since `state` stays `"chapter"` across chapter boundaries, `syncMusic()`'s live-duck path only handled re-rooting for the "play" theme, so the pad was stuck on the previous chapter's note while the melody generator jumped to the new chapter's scale. Both fixed in `game.js`; still needs an ear on the result.)*
